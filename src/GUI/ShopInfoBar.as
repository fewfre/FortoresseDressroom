package GUI
{
	...
	public class ShopInfoBar extends MovieClip
	{
		...
		private function _updateID() : void {
			this.Text.text = "ID: "+_getDataIDText();
		}
		...
		internal function saveSprite(pEvent:flash.events.Event) : void
		{
			...
			pName = "shop-"+data.type+_getDataIDText()+".png";
		...
		private function _getDataIDText() : String {
			if(data is SkinData) {
				return data.id + (data.id <= 5 ? ((data as SkinData).gender == GENDER.MALE ? "M" : "F") : "");
			} else {
				return data.id;
			}
		}
